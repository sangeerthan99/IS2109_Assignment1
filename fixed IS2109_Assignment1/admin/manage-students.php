...
if(isset($_GET['del']))
      ...
      if (isset($_GET['pass'])) {
        ...
        $hashed_password = password_hash($password . $salt, PASSWORD_DEFAULT); // hash password with bcrypt and salt
        $stmt = mysqli_prepare($con, "UPDATE students SET password=?, salt=? WHERE StudentRegno=?");
        mysqli_stmt_bind_param($stmt, "sss", $hashed_password, $salt, $_GET['id']);
        if (mysqli_stmt_execute($stmt)) {
            $_SESSION['delmsg'] = "Password reset. New password is Test@123";
        } else {
            $_SESSION['delmsg'] = "Error resetting password";
        }
